...
namespace Sistema_Inventario.Areas.Admin.Controllers
{
    ...
    public class ProductoController : Controller
    {
        ...
        public async Task<IActionResult> Upsert(ProductoVM productoVM)
        {
            ...
            {
                //Variable que cacha la imagen que biene de la vista==>files y la siguiente especifica la ruta donde se grabara la imagen
                ...
                if (productoVM.Producto.Id == 0)
                {
                    ...
                    //Se compleyta la ruta correcta
                    string upload = webRootPath + DS.ImagenRuta;

                    //Se crea Un Id Unico para el nombre de la imagen
                    string fileName = Guid.NewGuid().ToString();

                    //se obtiene la Exension del archivo
                    string extension = Path.GetExtension(files[0].FileName);

                    //Creando el Archivo o copiando a memoria
